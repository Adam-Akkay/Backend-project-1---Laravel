...
@section('content')
<div class="max-w-2xl">
    <h1 class="text-3xl font-bold mb-6">Nieuw FAQ item</h1>
        ...
        <form method="POST" action="{{ route('admin.faq-items.store') }}">
            ...
            <div class="mb-4">
                ...
                <textarea name="answer" id="answer" rows="6" required
                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('answer') border-red-500 @enderror">{{ old('answer') }}</textarea>
                @error('answer')
                    <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                @enderror
            </div>
